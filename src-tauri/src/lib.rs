...
#[tauri::command]
fn set_onboarding_step(app: tauri::AppHandle, step: String) {
...
#[tauri::command]
fn get_onboarding_step(app: tauri::AppHandle) -> String {
...
#[tauri::command]
fn set_onboarding_ptt(active: bool) {
    crate::input::ONBOARDING_PTT.store(active, Ordering::SeqCst);
    klog!(ptt, info, active = active, "onboarding ptt ownership");
}
...
fn focus_onboarding_window(app: &tauri::AppHandle) {
    ...
    let _ = app.clone().run_on_main_thread(move || {
        ...
        let _ = win.show();
        let _ = win.set_focus();
        klog!(auth, info, "focused onboarding window after auth callback");
    });
}
            ...
            // Accessory / no-Space-switch design and never yanks the user's focus.
            {
                ...
                app.deep_link().on_open_url(move |event| {
                    for url in event.urls() {
                        ...
                        focus_onboarding_window(&handle);
                        let code = url
                            .query_pairs()
                            .find(|(k, _)| k == "code")
