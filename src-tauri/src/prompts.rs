...
pub(crate) fn gate_system_prompt(skills_block: &str) -> String {
    let mut lines: Vec<String> = vec![
        ...
        "EXCEPTION — when the user reports FINISHING a step (\"done\", \"I've set it\", \"what's next\"), the thing they name is what they just COMPLETED, not what you're about to find. Echoing it sounds a turn behind. Look forward instead: \"Nice — let me line up what's next.\" NOT \"Let me find that corner radius field.\" Echo their words only when they ask about something specific.".to_string(),
