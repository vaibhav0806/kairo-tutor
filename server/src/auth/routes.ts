...
export function callbackPage(deepLink: string | null): string {
...
<head>
  ...
  <title>${success ? 'Signed in' : 'Sign-in needs attention'} · Kairo</title>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .dot{width:8px;height:8px;border-radius:50%;background:${success ? 'var(--verify)' : 'var(--rose)'};
      box-shadow:0 0 0 4px ${success ? 'rgb(184 243 74 / 18%)' : 'rgb(255 121 111 / 15%)'}}
  ...
  <div class="shell">
    ...
    <main>
      <header><div class="brand">${mark}<span>kairo</span></div>
        <div class="state"><span class="dot"></span>${success ? 'Secure sign-in complete' : 'Sign-in interrupted'}</div></header>
      <section class="content">
        <div class="kicker">${success ? 'Welcome back' : 'One more try'}</div>
        <h1>${success ? 'You’re in. Kairo is ready.' : 'That sign-in didn’t finish.'}</h1>
      ...
      <footer>
        ...
        <p class="hint">${success ? 'This tab can be closed<br>after Kairo opens.' : 'No account changes<br>were made.'}</p>
      </footer>
    </main>
  </div>
